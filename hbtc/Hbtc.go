...
const (
	GLOBAL_API_BASE_URL = "https://api.hbtc.com"
...
type ExchangeInfo struct {
	...
	ExchangeFilters []interface{} `json:"brokerFilters,omitempty"`
...
func (bn *Hbtc) buildParamsSigned(postForm *url.Values) error {
...
func New(client *http.Client, api_key, secret_key string) *Hbtc {
...
func NewWithConfig(config *APIConfig) *Hbtc {
	if config.Endpoint == "" {
	...
	bn := &Hbtc{
		...
		apiV1:      config.Endpoint + "/openapi/quote/v1/",
		apiV3:      config.Endpoint + "/openapi/v1/",
...
func (bn *Hbtc) GetExchangeName() string {
	return HBTC
}

func (bn *Hbtc) Ping() bool {
...
func (bn *Hbtc) setTimeOffset() error {
...
func (bn *Hbtc) GetTicker(currency CurrencyPair) (*Ticker, error) {
	tickerUri := bn.apiV1 + fmt.Sprintf(TICKER_URI, currency.ToSymbol(""))
...
func (bn *Hbtc) GetDepth(size int, currencyPair CurrencyPair) (*Depth, error) {
	if size <= 5 {
		size = 5
	} else if size <= 10 {
		size = 10
	...
	apiUrl := fmt.Sprintf(bn.apiV1+DEPTH_URI, currencyPair.ToSymbol(""), size)
...
func (bn *Hbtc) placeOrder(amount, price string, pair CurrencyPair, orderType, orderSide string) (*Order, error) {
	...
	resp, err := HttpPostForm2(bn.httpClient, path, params,
		map[string]string{"X-BH-APIKEY": bn.accessKey})
	if err != nil {
		return nil, err
	}
...
func (bn *Hbtc) GetAccount() (*Account, error) {
	params := url.Values{}
	bn.buildParamsSigned(&params)
	path := bn.apiV3 + ACCOUNT_URI + params.Encode()
	respmap, err := HttpGet2(bn.httpClient, path, map[string]string{"X-BH-APIKEY": bn.accessKey})
	if err != nil {
		return nil, err
	}
...
func (bn *Hbtc) LimitBuy(amount, price string, currencyPair CurrencyPair, opt ...LimitOrderOptionalParameter) (*Order, error) {
	return bn.placeOrder(amount, price, currencyPair, "LIMIT", "BUY")
}

func (bn *Hbtc) LimitSell(amount, price string, currencyPair CurrencyPair, opt ...LimitOrderOptionalParameter) (*Order, error) {
	return bn.placeOrder(amount, price, currencyPair, "LIMIT", "SELL")
}

func (bn *Hbtc) MarketBuy(amount, price string, currencyPair CurrencyPair) (*Order, error) {
	return bn.placeOrder(amount, price, currencyPair, "MARKET", "BUY")
}

func (bn *Hbtc) MarketSell(amount, price string, currencyPair CurrencyPair) (*Order, error) {
	return bn.placeOrder(amount, price, currencyPair, "MARKET", "SELL")
}

func (bn *Hbtc) CancelOrder(orderId string, currencyPair CurrencyPair) (bool, error) {
	path := bn.apiV3 + ORDER_URI
	params := url.Values{}
	...
	resp, err := HttpDeleteForm(bn.httpClient, path, params, map[string]string{"X-BH-APIKEY": bn.accessKey})
...
func (bn *Hbtc) GetOneOrder(orderId string, currencyPair CurrencyPair) (*Order, error) {
	params := url.Values{}
	params.Set("symbol", currencyPair.ToSymbol(""))
	if orderId != "" {
		params.Set("orderId", orderId)
...
func (bn *Hbtc) GetUnfinishOrders(currencyPair CurrencyPair) ([]Order, error) {
	params := url.Values{}
	params.Set("symbol", currencyPair.ToSymbol(""))
	...
	respmap, err := HttpGet3(bn.httpClient, path, map[string]string{"X-BH-APIKEY": bn.accessKey})
	if err != nil {
		return nil, err
	}
...
func (bn *Hbtc) GetKlineRecords(currency CurrencyPair, period, size, since int) ([]Kline, error) {
	params := url.Values{}
...
//非个人，整个交易所的交易记录
//注意：since is fromId
func (bn *Hbtc) GetTrades(currencyPair CurrencyPair, since int64) ([]Trade, error) {
	param := url.Values{}
...
func (bn *Hbtc) GetOrderHistorys(currency CurrencyPair, currentPage, pageSize int) ([]Order, error) {
	params := url.Values{}
	params.Set("symbol", currency.ToSymbol(""))
...
func (bn *Hbtc) toCurrencyPair(symbol string) CurrencyPair {
...
func (bn *Hbtc) GetExchangeInfo() (*ExchangeInfo, error) {
	resp, err := HttpGet5(bn.httpClient, bn.apiV3+"brokerInfo", nil)
	if err != nil {
		return nil, err
	}
	info := &ExchangeInfo{}
...
func (bn *Hbtc) GetTradeSymbol(currencyPair CurrencyPair) (*TradeSymbol, error) {
	if bn.ExchangeInfo == nil {
		var err error
		bn.ExchangeInfo, err = bn.GetExchangeInfo()
		if err != nil {
